...
@property (nonatomic, strong) SeanSpeedControl *controlView;
...
- (void)changeVideo:(UIButton *)sender {
    ...
    [self.controlView setPlayerRate:0.5];
...
- (void)nextClick:(UIButton *)sender {
    [self.controlView setPlayerRate:1.5];
//    if (!self.player.isLastAssetURL) {
//        [self.player playTheNext];
//        NSString *title = [NSString stringWithFormat:@"视频标题%zd",self.player.currentPlayIndex];
...
- (ZFPlayerControlView *)controlView {
    if (!_controlView) {
        _controlView = [SeanSpeedControl new];
        _controlView.fastViewAnimated = YES;
        _controlView.autoHiddenTimeInterval = 5;
        _controlView.autoFadeTimeInterval = 0.5;
        _controlView.prepareShowLoading = YES;
        _controlView.prepareShowControlView = YES;
        _controlView.isShowSpeed = YES;
    }
    return _controlView;
}
...
- (UIButton *)changeBtn {
    if (!_changeBtn) {
        ...
        [_changeBtn setTitle:@"0.8倍播放" forState:UIControlStateNormal];
...
- (UIButton *)nextBtn {
    if (!_nextBtn) {
        ...
        [_nextBtn setTitle:@"1.5倍播放" forState:UIControlStateNormal];
