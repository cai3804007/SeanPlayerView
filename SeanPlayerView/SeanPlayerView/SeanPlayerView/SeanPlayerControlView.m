...
@interface SeanPlayerControlView ()<ZFSliderViewDelegate>
/// 底部工具栏
...
@property (nonatomic,strong) UIView *speedContentView;

@property (nonatomic,strong) UIButton *speedButton;
@end


@implementation SeanPlayerControlView
...
- (void)speedButtonClick:(UIButton *)sender{
    if (self.speedButtonBlock) {
        self.speedButtonBlock();
    }
}
...
- (void)layoutSubviews {
    ...
    self.playOrPauseBtn.center = CGPointMake(self.zf_width/2.0, self.zf_height/2.0);
    ...
    min_y = 0;
    if (self.isShowRate) {
        min_x = self.fullScreenBtn.zf_left - min_w - 30;
    }
    ...
    if (_isShowRate) {
        min_w = 30;
        min_h = 30;
        min_x = self.fullScreenBtn.zf_x - 30;
        min_y = 0;
        self.speedButton.frame = CGRectMake(min_x, min_y, min_w, min_h);
        self.speedButton.zf_centerY = self.fullScreenBtn.zf_centerY;
    }
}
...
-(UIView *)speedContentView{
    if (!_speedContentView) {
        _speedContentView = [[UIView alloc] init];
        _speedContentView.userInteractionEnabled = YES;
    }
    return _bgImgView;
}
